...
predictors = c(reporters[reporters!="p.ERK"])
response = "p.ERK"
...
pred.p.ERK <- predict(rangerres0, data = pERK_validation_data)


pERK_validation_data %>% select(cell_line,treatment,time,time_course,p.ERK) %>%
bind_cols(p.ERK.pred = pred.p.ERK$predictions) %>%
	mutate(time = ifelse(is.na(time),0,as.numeric(as.character(time)))) %>%
	ggplot() +
	...
	facet_grid(treatment~cell_line) + scale_color_manual(values = c("data" = "black", "pred" = "red"))
